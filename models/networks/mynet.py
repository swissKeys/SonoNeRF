...
class ResNeXtBottleneck(nn.Module):
    ...
    def forward(self, x):
        # print('bottleneck in {}'.format(x.shape))
        residual = x
        ...
        out += residual
        out = self.relu(out)
        # print('bottleneck out {}'.format(out.shape))
        # print('*' * 50)
        return out
...
class ResNeXt(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv2 = nn.Conv3d(in_channels=512, out_channels=128, kernel_size=(5, 3, 3),
                               stride=(1, 2, 2), padding=(2, 1, 1))

        self.fc = nn.Linear(cardinality * 32 * block.expansion, num_classes)
        self.dropout1 = nn.Dropout(p=0.25, inplace=False)


        # self.at_bn1 = nn.BatchNorm3d(1024)
        self.attention = nn.Sequential(
            nn.BatchNorm3d(128),
            nn.Conv3d(in_channels=128, out_channels=64, kernel_size=(5, 3, 3), stride=(1, 1, 1), padding=(2, 1, 1)),
            nn.ReLU(),
            nn.Conv3d(in_channels=64, out_channels=1, kernel_size=(5, 3, 3), stride=(1, 1, 1), padding=(2, 1, 1)),
            nn.BatchNorm3d(1),
            nn.Sigmoid()
        )
