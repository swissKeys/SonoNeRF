...
class VisualizeSequence():
    def __init__(self, case_id):
        ...
        for frame_id in range(self.frames_num):
            ...
            frame_img = train_network_poses.data_transform(frame_img)
...
def sample_ids(valid_ids, neighbour_num, sample_option='skip', random_reverse_prob=0, self_prob=0):
    """
    This function gives different sampling strategies.
    :param valid_ids: List of valid image IDs
    ...
    :return:
    """
    slice_num = len(valid_ids)  # number of total slices of a case is determined by the valid_ids list
    skip_ratio = 3
    ...
    if sample_option == 'skip':
        start_id = np.random.choice(valid_ids[:-skip_ratio*neighbour_num])
        ...
        start_id = np.random.choice(valid_ids[:-neighbour_num])
    ...
    if random.uniform(0, 1) < self_prob:
        ...
        slice_id = np.random.choice(valid_ids)
...
def center_crop():
    ...
    for i in frame_list:
        ...
        crop = train_network_poses.data_transform(input_img=frame_img)
